...
FOR UPDATE
USING (id = auth.uid())
WITH CHECK (id = auth.uid() AND (role IS NULL OR role = OLD.role OR has_role(auth.uid(), 'admin'::app_role)));

-- Secure user_roles table - only admins can assign roles
DROP POLICY IF EXISTS "insert_subscription" ON public.user_roles;
...
-- Keep the select policy for users to see their own roles
-- (select_own_roles policy already exists)