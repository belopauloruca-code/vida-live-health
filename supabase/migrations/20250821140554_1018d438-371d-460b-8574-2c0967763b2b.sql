...
-- Create new secure profile update policy that prevents role changes by regular users
CREATE POLICY "update_own_profile_no_role_change" ON public.profiles
FOR UPDATE
USING (id = auth.uid())
WITH CHECK (id = auth.uid());

-- Add separate policy for admin role changes
CREATE POLICY "admin_can_update_roles" ON public.profiles
FOR UPDATE
