...
export const DashboardPage: React.FC = () => {
  ...
  const [mealPlanStats, setMealPlanStats] = useState({
    meals: 0,
    recipes: 0,
    duration: 0
  });
  ...
  const loadHydrationToday = async () => {
    ...
    const {
      data
    } = await supabase.from('hydration_logs').select('amount_ml').eq('user_id', user.id).gte('ts', `${today}T00:00:00`).lte('ts', `${today}T23:59:59`);
    if (data) {
  ...
  const loadMealPlanStats = async () => {
    ...
    const {
      data: mealPlans,
      error
    } = await supabase.from('meal_plans').select(`
        *,
        meal_plan_items(*)
  ...
  const setupRealtimeSubscriptions = () => {
    ...
    const hydrationChannel = supabase.channel('hydration-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
    ...
    const mealPlanChannel = supabase.channel('meal-plan-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'meal_plans',
      filter: `user_id=eq.${user.id}`
  ...
  return <div className="min-h-screen bg-background pb-safe-bottom-nav">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6 flex justify-between items-start">
          <BrandHeader title={`${t('dashboard.welcome')}, ${profile?.name || 'Usuário'}!`} subtitle="Como está sua jornada hoje?" showLogo={false} />
          <div className="flex items-center gap-2">
            <LanguageSwitcher variant="compact" />
            <Button variant="outline" size="sm" onClick={handleLogout} disabled={isLoggingOut} className="flex items-center gap-2 disabled:opacity-50">
        ...
        <Card className="mb-6 border-blue-100">
          ...
          <CardContent>
            <div className="flex items-center justify-center mb-4">
              <div className="relative w-32 h-32">
                <svg className="w-32 h-32 transform -rotate-90" viewBox="0 0 120 120">
                  ...
                  <circle cx="60" cy="60" r="50" stroke="#3b82f6" strokeWidth="8" fill="none" strokeDasharray={`${2 * Math.PI * 50}`} strokeDashoffset={`${2 * Math.PI * 50 * (1 - waterProgress / 100)}`} className="transition-all duration-300" />
                </svg>
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {[150, 200, 300, 500].map(amount => <Button key={amount} size="sm" variant="outline" onClick={() => addWater(amount)} className="border-blue-200 hover:bg-blue-50">
                  {amount}ml
                </Button>)}
            </div>
            
            <div className="flex justify-between text-sm text-gray-600">
              <span>{t('dashboard.hydration.cupsRemaining', {
                cups: cupsRemaining
              })}</span>
              <span>{t('dashboard.hydration.cupsDrunk', {
                cups: cupsDrunk
              })}</span>
            </div>
          </CardContent>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {hasActiveSubscription ? <Button variant="outline" className="h-16 border-green-200 hover:bg-green-50" onClick={() => window.location.href = '/meal-plans'}>
              {t('dashboard.quickActions.viewMealPlans')}
            </Button> : <div className="relative h-16 bg-cover bg-center bg-no-repeat rounded-lg overflow-hidden cursor-pointer hover:scale-105 transition-transform duration-200" style={{
          ...
          <Button variant="outline" className="h-16 border-green-200 hover:bg-green-50" onClick={() => window.location.href = '/exercises'}>
